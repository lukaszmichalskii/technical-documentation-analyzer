...
class TestGoogleKnowledgeGraphSearch(unittest.TestCase):
    ...
    def test_search(self):
        ...
        expected = (['The cat is a domestic species of small carnivorous mammal. It is the only domesticated species '
                     'in the family Felidae and is commonly referred to as the domestic cat or house cat to '
                     'distinguish it from the wild members of the family. '], [['Thing']],
                    ['https://en.wikipedia.org/wiki/Cat'])
        try:
            actual = google_kgs.google_search(query, api_key, limit, ner=True)
