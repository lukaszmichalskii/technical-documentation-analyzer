...
class TestSVO(unittest.TestCase):
    ...
    def test_create_svo_triples_large(self):
        text = (
            "Given the real time images from two stereo ZED cameras, camera recognition detects landmarks and "
    ...
    def test_create_svo_lists(self):
        text = (
            "First version of cone detection module successfully detects cones from input images based on the "
            "trained network."
        )
        doc = nlp(text)
        expected = {
            ...
            "verbs": [("detect", 52)],
            "obj": [
                ("cone detection module", 17),
                ("cones", 60),
                ("input images", 71),
                ("the trained network", 93),
            ],
        }
