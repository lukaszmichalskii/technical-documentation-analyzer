...
Observations can either be observations with known data associations: every observation can easily be associated with given landmark using some unique property that distinguishes this landmark from others or observations with unknown data associations: one can not associate observation with any landmark.
Associations have to be made using current location and observation coordinates.
All cones at track are the same (3 colors). This means that we can not associate observation on observation alone. This means that Formula Student SLAM problem is a SLAM with unknown data associations.
...
There was no possibility of running SLAM in debug mode as it could not run in real-time.
...
Unit test for other functionality than k-d trees was not implemented as SLAM algorithms are highly probabilistic and modeling unit tests for those algorithms does not make sense.
